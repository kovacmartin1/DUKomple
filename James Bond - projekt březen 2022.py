...
print("Upozorneni: Po celou dobu hry odpovidej pouze moznymi odpovedmi. Jinak hrozi ztrata progresu.")

jmeno = input("Ahoj, jak se jmenujes? ")

while True:
    odpoved = input(f"Odted se jmenujes Bond, {jmeno} Bond. Jsi agent tajne sluzby MI6 a jsi na ceste do Brazilie, kde zacina tvoje mise (ziskat ukradenou kralovskou korunu). Chces se ji zucastnit? ano/ne ")
    if odpoved == "ano":
        odpoved = input("Jaky dopravni prostredek si pro cestu na misi vyberes? lod/letadlo ")        
        if odpoved == "lod":
            odpoved = input("Jedes lodi a asi po 4 hodinach cesty Atlantickym oceanem vidis pred sebou ledovou kru. Tvoje lod ma vybavu pro rozrazeni ledu. Jedes do nej? ano/ne ")
            if odpoved == "ano":
                print("Bohuzel byl led moc tvrdy. Mas diru v lodi a zacnes se potapet. Game over. ")
                break
            ...
            odpoved = input("Letis letadlem a asi po 2 hodinach cesty nad Atlantickym oceanem vidis na hladine namorni bitvu. Tvoje letadlo ma vybavu pro boj na vode. Zapojis se? ano/ne ")
            if odpoved == "ano":
